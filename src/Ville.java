...
public class Ville {
	...
	public void addVilleSort(Ville v) { // Fonction qui permet d'ajouter une
										// ville comme voisine
		this.voisSort.add(v);
	}

	public boolean voisinSort(Ville v) {// fonction qui vérifie si v est une
										// ville voisine
	...
	public boolean pasChemin() { // Foction qui Permet de verifier si la ville
									// actuelle ne mène a aucune autre ville
	...
	public boolean existe3Chemin() {// Fonction qui permet de verifier
									// l'existence de 3 Villes voisines
									// exactement
		int cpt = 0;
	...
	public boolean voisinDifferents() { // Fonction qui permet de verifier si
										// toutes le villes voisines sont
										// differentes entre elles
	...
	public boolean plusFleurie() {// Fonction qui permet de verifier que la
									// ville actuelle est plus fleurie que ses
									// voisines
	...
	public boolean cheminExiste(Ville fin, Ville... chemin) { // Fonction qui
																// permet de
																// verifier
																// l'existence
																// d'un chemin
	...
	public boolean existeChemin(Ville fin) {// Fonciton qui permet de verifier
											// l'existence d'un chemin vers une
											// ville donnée
	...
	public Ville recherche(String nom) { // Fonction qui permet de rechercher
											// une ville avec son nom
	...
	public boolean supprime() { // Fonction qui permet de supprimer La ville
								// actuelle
	...
	public void modifieVille() { // Fonction qui permet de modifier les
									// informations d'une ville
		int choice = 1;
		Scanner in = new Scanner(System.in);

		do {
			System.out.printf(
					"Que voulez-vous changer?: %n 1) Nom 2) Wilaya 3) Superficie 4) Type de ville 0) Annuler.%n Votre choix: ");
			choice = in.nextInt();
		...
		case 4:
			...
			do {
				System.out.printf(
						"Entrez le nouveau type:  1) Agricole, 2) Touristique, 3) Industrielle, 4) Ordinaire.%n Votre choix: ");
				type = in.nextInt();
				if (type < 1 || type > 4)
	...
	/*
	 * public void afficheReseauVille(){ this.afficheReseauVille( 0, 0); }
	 */
	// Fonction d'affichage des villes (revursive)
	public void afficheReseauVille(int numSpace, int i) {
		...
		// ajouter la taille de la ville crnt pour mieux afficher ses voisins
		numSpace += this.getNom().length();

		// Si la ville crnt a des voisins
		if (!this.pasChemin()) {

			// space == on est en train d'afficher dans un nvx ligne
			boolean space = false;

			// Parcours des voisins de la ville
			for (Ville temp : this.getVoisSort()) {

				if (temp != null) {
					// si on doit ajouter des espaces avant le nom de la ville
					if (space) {
						...
						// on a deja afficher la 1ere ville
						space = !space;

					}

					System.out.print(" => ");
					// appelle recursive en changant les params
